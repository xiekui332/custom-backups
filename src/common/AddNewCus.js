...
class AddNewCus extends Component{
            ...
            <AddWrapper className={isAdd || cusEdit?"isShow overflow":"isHide overflow"}>
                ...
                <AddContent>
                    <Fragment>
                        ...
                        <AddItem className='clear-fix'>
                            <AddTitle><span></span>客户号</AddTitle>
                                <input
                                    className="add-input"
                                    placeholder="请输入"
                                    ref = {(input) => {this.customerNoEl = input}}
                                    value={customerDetail && customerDetail.customerNo?customerDetail.customerNo:""}
                                    onChange={() => {this.setValue(this.customerNoEl, 41)}}
                                />
                            <p></p>
                        </AddItem>
                        ...
                        {/* 新增客户类型 S */}
                        <AddItem className='clear-fix'>
                            <AddTitle><span></span>客户类型</AddTitle>
                            <Select
                                showSearch
                                ...
                                onChange={(value) => {this.handleCustomerType(value, customerDetail)}}
                                value={custTypeVal}
                            >
                                {
    ...
    componentDidMount() {
        ...
        if(this.props.customerDetail) {
            const { customerType } = this.props.customerDetail
            let type = ''
    ...
    // 客户类型
    handleCustomerType(value, customerDetail) {
        this.setState({
            custTypeVal:value
        })
    }
    ...
    // 新建客户保存
    handleSaveData(
            cusNameEl, cusIdcardEl, cusMobileEl, cusAddressEl, customerNoEl, cusConEl, cusCompanyEl, value, kindSelected, cusMoneyEl, fileList, attachFile, 
            customerId,  
            cusadddizhiEl,
            cusaddryEl,
            cusaddmjEl,
        ...
        if(!cusNameEl.value) {
            ...
            params.customerNo = customerNoEl.value
            params.customerType = this.state.custTypeVal    // 客户类型
    ...
    setValue(el, type, cusEdit) {
        ...
        if(type === 1) {
            ...
            customerDetail.customerNo = el.value
        }
        else if(type === 5) {
